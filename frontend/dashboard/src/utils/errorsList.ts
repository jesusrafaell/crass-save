import { IError, IErrors } from '@/models';
...
const listCodeErrors: {
	...
	invalidLicensePlate: { code: 'R004V', desc: 'invalid LicensePlate: must be >= 6 alphanumeric characters' },
	invalidPolicyNumber: { code: 'R006V', desc: 'invalid policy number: must be alphanumeric and >= 3 characters' },
...
function getErrorDescription(code: string): string | undefined {
	const normalizedCode = code.trim().toUpperCase(); // Normaliza el código (quita espacios y convierte a mayúsculas)
	const errorEntry = Object.values(listCodeErrors).find((entry) => entry.code === normalizedCode);
