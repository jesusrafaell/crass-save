...
function ChangePassword() {
		...
		if (!hasRun.current) {
			const verifyToken = async () => {
				try {
					...
					if (token) {
						const response = await fetch(`${import.meta.env.VITE_URL_API}/v1/api/verify-token/change-password`, {
							method: 'POST',
							headers: {
	...
	const onSubmit = async (password: string) => {
		try {
			...
			const token = urlParts[urlParts.length - 1];
			if (token) {
				const response = await fetch(`${import.meta.env.VITE_URL_API}/v1/api/auth/forgot-password`, {
					method: 'PUT',
					headers: {
						'Content-Type': 'application/json',
					},
				...
				<div className='min-h-screen bg-gray-100 flex flex-col justify-center sm:py-12'>
					<div className='mx-auto container '>
						...
						<div className='bg-white shadow w-full rounded-lg divide-y divide-gray-200 min-w-[350px]'>
							{!isChangedPassword ? <PasswordForm onSubmit={onSubmit} load={load} /> : <SucessMessage />}
						</div>
					</div>
				</div>
			</div>
